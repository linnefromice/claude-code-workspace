...
差分翻訳の全処理が完了したら、変更タイプごとにファイル単位のサマリを出力する。
このサマリは作業記録およびレビュー用途で使用される。

#### 出力フォーマット

以下の構成で、全処理ファイルのサマリをユーザーに提示する：

**削除(D)**:

| # | 削除ファイル | 理由 |
|---|-------------|------|
| 1 | `（旧パス）` | （削除理由: リネーム先、ソース側削除 等） |

**リネーム(R)**:

| # | 旧パス → 新パス | 類似度 | 主な内容変更 |
|---|----------------|--------|-------------|
| 1 | `（旧パス）` → `（新パス）` | RXXX | （変更の要約） |

**更新(M)**:

| # | ファイル | 主な変更内容 |
|---|---------|-------------|
| 1 | `（ファイルパス）` | （ソース側の差分に基づく変更の要約） |

**新規翻訳(A)**:

| # | ファイル | 行数 | 概要 |
|---|---------|------|------|
| 1 | `（ファイルパス）` | XXX | （ファイルの概要: 目的、主な内容、特徴） |

#### 記載ルール

- **削除・リネーム**: 旧パスと理由/移動先を明記
- **更新**: ソース側の diff に基づき、翻訳に反映した変更点を簡潔に記載
- **新規翻訳**: ファイルの目的・主な内容・行数を記載。翻訳対象外（スクリプト等）は「翻訳対象外」と注記
- **除外ファイル**: 汎用化フィルタで除外したファイルがある場合は、除外一覧も別途記載
