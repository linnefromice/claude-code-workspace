...
<!-- CUSTOMIZE: 以下は参考例です。自分のプロジェクト情報に置き換えてください -->
実際のプロダクションアプリケーションに基づく例。

---

## 使用タイミング

対象の特定プロジェクトで作業する際にこのスキルを参照。プロジェクトスキルには以下が含まれる:
- アーキテクチャ概要
- ファイル構造
- コードパターン
- テスト要件
...
- [ ] ローカルで全テスト合格
- [ ] `npm run build`成功（フロントエンド）
- [ ] `poetry run pytest`合格（バックエンド）
- [ ] ハードコードされたシークレットなし
- [ ] 環境変数のドキュメント化
- [ ] データベースマイグレーション準備完了
...
1. **絵文字禁止** - コード、コメント、ドキュメントに
2. **イミュータビリティ** - オブジェクトや配列を直接変更しない
3. **TDD** - 実装前にテストを書く
4. **80%カバレッジ** - 最低限
5. **多くの小さなファイル** - 200-400行が標準、800行が最大
6. **console.log禁止** - プロダクションコードに
